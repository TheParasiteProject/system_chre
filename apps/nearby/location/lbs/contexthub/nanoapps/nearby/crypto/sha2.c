...
static void sha2processBlock(struct Sha2Context *ctx)
{
    ...
    for (;i < SHA2_WORDS_CTX_SIZE; i++) {
        uint32_t s0 = ror(ctx->w[i-15], 7) ^ ror(ctx->w[i-15], 18) ^ (ctx->w[i-15] >> 3);
        uint32_t s1 = ror(ctx->w[i-2], 17) ^ ror(ctx->w[i-2], 19) ^ (ctx->w[i-2] >> 10);
...
void sha2processBytes(struct Sha2Context *ctx, const void *inData, size_t dataLen)
{
...
void sha2finish(struct Sha2Context *ctx, void *outHash, uint32_t hashLen)
{
    ...
    //append the zeroes
    appendend = 0;
    while (ctx->bufBytesUsed != 56)
        sha2processBytes(ctx, &appendend, 1);

    //append the length in bits (we can safely write into context since we're sure where to write to (we're definitely 56-bytes into a block)
...
void sha256(const void *inData, const uint32_t dataLen,
            void *outHash, const uint32_t hashLen)
{
